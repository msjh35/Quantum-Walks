...
M = 15 #Size of rectangle in x direction
N = 15 #Size of rectangle in y direction 
...
ket = braket1.ket1
bra = braket1.bra1
Ix = np.identity(2**(number_qubits_x))
Iy = np.identity(2**(number_qubits_y))

# Periodic boundary for x-co
Px = Ix - ket(0)*bra(0) - ket(M+1)*bra(M+1) + ket(M-1)*bra(M+1) + ket(2)*bra(0)

# Boundary for y-co
Py = Iy - ket(0)*bra(0) - ket(N+1)*bra(N+1) + ket(N-1)*bra(N+1) + ket(2)*bra(0)
...
def graph(final):
    ...
    dz = norm_p
    scaled_x = [x for x in x_pos]    
    scaled_y = [y for y in y_pos]
    ...
    if plot3D == True:
        ...
        ax.bar3d(scaled_x, scaled_y, z_pos, dx, dy, dz, color=cmap.to_rgba(norm_p))  
        ax.set_xlabel('Position $x_1$')
        ax.set_ylabel('Position $x_2$')
        ax.set_zlabel('Probability');
       # plt.savefig('/Users/Elena/Library/Mobile Documents/com~apple~CloudDocs/Documents/Durham/Dissertation/Plots/Rectangle_15x15.png')
        plt.show()

    elif plot3D == False:
        arr = np.zeros(((max(x_pos)+1), max(y_pos)+1))
        arr[x_pos, y_pos]=norm_p
