...
ket2 = braket2.ket2
bra2 = braket2.bra2
I1 = np.identity(2**(number_qubits_y))
I2 = np.identity(2**(number_qubits_x+number_qubits_y))

# TPBC for y-co 
Pr = []
for x in range(1, M+1):
    el = - ket2(x, N+1)*bra2(x, N+1) - ket2(x,0)*bra2(x,0) + ket2(M-x+1,1)*bra2(x, N+1)+ket2(M-x+1, N)*bra2(x,0)
    Pr.append(el)
Prr = sum(Pr)
P = I2 + Prr

# PBC for x-co
Px = I1 - ket1(M+1)*bra1(M+1) - ket1(0)*bra1(0) + ket1(1)*bra1(M+1) + ket1(M)*bra1(0)

class Projection(cirq.Gate):
    def __init__(self):
        super(Projection, self)
...
for number_steps in range(6, 61):
    ...
    def generate_walk(number_qubits_x, number_qubits_y, number_steps, sample_number):
        ...
        for j in range(0, number_steps):
            ...
            circuit.append(P.on(cirq.GridQubit(0, 0), cirq.GridQubit(0, 1),cirq.GridQubit(0, 2), cirq.GridQubit(0, 3),cirq.GridQubit(0, 4),cirq.GridQubit(0, 5), cirq.GridQubit(1, 0), cirq.GridQubit(1, 1),cirq.GridQubit(1, 2), cirq.GridQubit(1, 3),cirq.GridQubit(1, 4),cirq.GridQubit(1, 5)))
    ...
    def graph(final):
        ...
        if plot3D == True:
            ...
            #plt.savefig('/Users/Elena/Documents/Python/2D-Walks/Plots/Prob Distribution for Initial Coin State 0.5(|0>+i|1>)(|0>-i|1>).png')
            plt.show()

        elif plot3D == False:
            arr = np.zeros(((max(y_pos)+1), max(x_pos)+1))
            arr[y_pos, x_pos]=norm_p
            ...
            plt.xlabel('x')
            plt.ylabel('y')
            plt.title('15 x 15 Klein Bottle')
            plt.savefig('/Users/Elena/Library/Mobile Documents/com~apple~CloudDocs/Documents/Durham/Dissertation/Plots/Heat_Maps/15x15/T='+str(number_steps)+'/Klein.png', dpi=300)
            #plt.savefig('/Users/Elena/Library/Mobile Documents/com~apple~CloudDocs/Documents/Durham/Dissertation/Plots/Heat_Maps/Klein/T='+str(number_steps)+'.png', dpi=300)
            plt.clf()
            #plt.show()
    graph(final)